...
#' @rdname process_data
setMethod("process_data", signature(
  data = 'DGEList',
  group_col = 'character',
...
function(data,
  ...
  data <- edgeR::DGEList(counts = Biobase::exprs(data),
...
function(data,
  ...
  data <- edgeR::DGEList(counts = SummarizedExperiment::assay(data, slot),
...
function(data,
  ...
  data <- edgeR::DGEList(counts = Seurat::GetAssayData(data, slot = slot),
                         samples = data@meta.data,
                         group = data@meta.data[[group_col]])
...
plot_diagnostics <- function(expr1, expr2, group_col, abl = 2) {
  ...
  stopifnot("expr1 must be matrix!" = is(expr1, "matrix") | is(expr1, "Matrix"),
  ...
  data1 <- tidyr::pivot_longer(as.data.frame(expr1),
                               ...
                               names_to = "Sample",
                               values_to = "logcounts")
  data1 <- dplyr::left_join(data1, data.frame(Sample = colnames(expr1),
                                              Group = group_col),
                            by = "Sample")
  data2 <- tidyr::pivot_longer(as.data.frame(expr2),
                               ...
                               names_to = "Sample",
                               values_to = "logcounts")
  data2 <- dplyr::left_join(data2, data.frame(Sample = colnames(expr2),
                                              Group = group_col),
                            by = "Sample")
