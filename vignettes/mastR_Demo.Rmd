...
- `msigdb_gobp_nk`: a 'GeneSetCollection' object, containing genesets with gene-set name matched to 'NATURAL_KILLER' in GO:BP MSigDB v7.4 database. More details `?mastR::msigdb_gobp_nk`.

- `NK_markers`: a combination of CIBERSORT LM7, LM22 and Huntington gene list with 114 genes. ([Curson et al. 2019](https://cancerimmunolres.aacrjournals.org/content/7/7/1162.long)).
...
There're `r length(MSig)` gene-sets in MSig, which is too many for visualization. Thus, we use function `merge_markers()` to merge all gene-sets into one 'GeneSet' object. (*Note*: Users can directly use the un-merged object for subsequent analyses.)
...
By setting `keep.top` and `keep.group`, the top n DEGs of specified DE comparisons can be kept (DEGs are ranked based on gene statistic setted by `Rank` param). In this case, if we want keep more distinguishable genes between NK and CD8+ T cells, we can set `keep.group = "CD8"` or `keep.group = "NK-CD8"` and use `keep.top = n` to specify how many top genes in that comparison should be kept.
...
Then intersect the signature genes `sig_ct` with the markers pool `Markers`, to constrain the final signature wthin the interested gene list. In this demo, only keep 'UP' regulated genes.
...
The advantage is that users can avoid over-normalizing or mis-correcting when carrying out data integration. And users can get more robust and conserved signature across datasets. It's better than "RemoveBatchEffect" sometimes.
...
Then use `pseudo_samples()` to get psudo-bulk data and create a DGEList.
