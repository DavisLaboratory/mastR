...
1)  generate from sources  
    a)  LM7/22 signature matrix for CIBERSORT  
    b)  MSigDB  
    c)  PanglaoDB  
    d)  Customized gene list  
...
Users can use `species`, `cat`, `subcat` and `pattern` to specify which gene-sets to be added.
...
Markers can also be generated from a customized gene list. Here we use a list of natural killer (NK) cell markers from Curson et al 2019 from [Curson's Publication](https://aacrjournals.org/cancerimmunolres/article/7/7/1162/469488/A-Gene-Signature-Predicting-Natural-Killer-Cell) as a customized gene list.
...
# Step 2. Identification and Refinement of Signature

## Signature Construction by Factors of Interest

------------------------------------------------------------------------

### Signature Identification for Target Group
...
`process_data()` in mastR does the genes filtration, data normalization, samples weighting, linear model fit and the computation of gene differential expression statistic as a single function call.
...
#### visualization of QC results before and after processing

After data processing, wcheck whether the data has been well filtered , normalized and fitted.

mastR provides visualization functions to compare the data before and after `process_data()` and see how well the QC has been carried out.
...
The density plot shows we successfully removed most of low quality genes which makes the distribution more normal-like.

RLE plot shows smaller deviation across samples, proving the success of normalization after processing.

While MDS doesn't show quite different result as it only uses top variable genes which should not change too much before and after processing.
...
Mean-variance trend can be plotted by `plot_mean_var()` to show the linear fit result. The droping trend in voom and the horizontal line in final model indicates the good fit of the data.
...
PCA can be plotted by `pca_matrix_plot()` to assess the variations in the data and weather normalization was able to remove batch effects. You can select how many top n PCs to be plot by param `n`.
...
#### signature genes selecton based on statistical analysis {#selection}

Now we get processed data with DE statistic for each gene in each comparison, then the proc_data can be passed into `select_sig()` function for further genes selection based on rank product scoring. Group specific signature is therefore determined.

`feature_selection`: choose whether to use rank product ('rankproduct') or not ('none') to select DEGs from multiple comparisons of DE analysis results. Default "auto" will perform "rankproduct" and switch to "none" if the number of final "UP" (up-regulated genes) and "DOWN" (down-regulated genes) genes are < 5.
...
***Note***: *All above steps from \@ref(process) to \@ref(selection) for screening group signature can be done by one integrated function `get_degs()`.*
...
#### Genrating signature from markers pool {#intersect-pool}
...
As we are only interested in up-regulated genes in NK cells in this demo, only `UP` DEGs are kept now.
...
pca_matrix_plot(proc_data$vfit$E[names(m_ct), ],
  ...
  n = 3
) + ggtitle("After intersection")
## before intersection
pca_matrix_plot(proc_data$vfit$E[names(deg_up), ],
  group_by = proc_data$samples$celltype.ch1,
  n = 3
) + ggtitle("Before intersection")
```
...
### Signature Refinement by Background Correction in Tissue (optional)
...
mastR provides a `remove_bg_snr()` function that can eliminate genes with strong signal in cancer cells or tissues, regarded as background noise. Users can input two datasets: one as the signal data (e.g. immune cell data) and the other as the background noise data (e.g. cancer cells or tissue cells data). Then, all genes with low SNR (signal to noise ratio) will be eliminated from the given markers list. This ensures that our immune signature remains unaffected by tumor purity.

The sig_data shoule always be the proc_data generated by `process_data()` function.

Based on the `bg_data` input format, this function can be used in 2 ways:

- use CCLE as `bg_data` input
- use customized data as `bg_data` input

#### using CCLE as cancer cell background data

The default setting of this `remove_bg_snr()` uses CCLE from [DepMap CCLE](https://depmap.org/portal/download/), containing RSEM quantified TPM data of 1392 cancer cell lines (until 2022.04).
...
`remove_bg_snr()` removes genes which are highly expressed in corresponding disease cell lines (in this case CRC adherent cell lines). This removes tumor purity as a factor in the identified signature markers.

Required params are as follows:

- `bg_data` can be either "CCLE" or expression dataset, e.g. matrix, eSet, DGEList, Seurat and so on.
- `sig_data` can be a matrix or DGEList, as signal data, always use processed data after `process_data()`.
- `b_group_col` param is the group vector of `bg_data` samples, or the name of group column.
- `b_target_group` param is an regex pattern to specify the samples of interest, e.g. choose 'colorectal' to select all colorectal adherent cell lines from CCLE.
- `s_group_col` is a vector or character, to specify the group of target_group in `sig_data`, or the name of group column.
- `s_target_group` is a regex pattern, to specify the target group of interest in `sig_data`.
- `markers` is the genes need to be filtered, should be gene symbols.

***Note***: Both of `bg_data` and `sig_data` should be log-normalized.
...
#### using customized dataset as background data

Other customized data is also accepted as `bg_data` in many formats: eSet, matrix, DGEList... But all input must be log-transformed expression data.

In this demo, we use a small imported dataset `ccle_crc_5` in mastR as a customized dataset, the expression of it is logTPM.

`ccle_crc_5` is a DGEList object only contains 5 CRC cell line samples from CCLE. More details for `help(ccle_crc_5)`.

```{r remove markers for cancer cell line}
data("ccle_crc_5")
ccle_crc_5[1:10, ]
...
But here, if we directly use the `m_ct` instead of `Markers` as input `markers` for function `remove_bg_snr()`, then `m_ccl` is already the intersection result of original `m_ccl` and `m_ct`, so we can skip this.

Now we get the final signature specific for both cell type and cancer type!

# Step 3. Visualization of Screening Results
