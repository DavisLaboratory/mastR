...
In this demo, we will be using the example of immune cell signature in the context of the tumor micro-environment as a practical case study to showcase the application of our approach.
...
Recent studies have highlighted the role of immune components in the TME in modulating tumor progression, making them attractive therapeutic targets. These components make up the tumor immune micro-environment (TIME), which is a subset of the TME. Subsequently, it's proved that tumor infiltrating lymphocytes (TILs) play an essential role in tumor progression, metastasis and treatment response.
...
**mastR**, **MA**rkers **S**creening **T**ool in **R**, is a package to automatically derive signature for specific group of interest in specific tissue.
...
As the gene-set names are too long, for better visualization the gene-set names are replaced with letters and shown later.
...
***Note***: External data is accepted in many formats (e.g. DGEList, eSet, matrix). All input data must be raw counts data or log-transformed expression data.

In this demo, we use the imported example data `im_data_6` from [GSE60424](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE60424), consisting of healthy individuals and immune cells. Downloaded using `GEOquery::getGEO()`.
...
An expression data with sample group labels (containing interested target group) is needed for `process_data()`.

Then it will return a `DGEList` object with filtered counts (unfiltered counts data will be saved under "original"), vfit (`limma::voom()` is done), tfit (`limma::lmFit()` and `limma::treat()` are done).

mastR provides the gene ID conversion feature using param `gene_id` when the gene ID of input data is not SYMBOL.
...
Now we save the expression data after limma::voom pipeline under the `proc_data` for subsequent analysis.
...
Now with the processed data `proc_data`, use `select_sig()` function to further select genes based on the DE results using rank product score. Group specific signature is therefore determined.
...
mastR also implements an additional feature to further increase the discriminative power of the signature between the fairly similar groups.

By setting `keep.top` and `keep.group`, the top n DEGs of specified DE comparisons can be kept (DEGs are ranked based on gene statistic set by `Rank` param). In this case, if we want keep more distinguishable genes between NK and CD8+ T cells, we can set `keep.group = "CD8"` or `keep.group = "NK-CD8"` and use `keep.top = n` to specify how many top genes in that comparison should be kept.


***Tips:***

All above steps from \@ref(process) to \@ref(selection) for refining group signature can be done by one integrated function `get_degs()`.
...
Then intersect the signature genes `sig_ct` with the markers pool `Markers`, to constrain the final signature within the interested gene list. In this demo, only keep 'UP' regulated genes.

Because the row names of `im_data_6` are ENSEMBL IDs while the markers are all gene SYMBOLs, we need to convert the IDs to the same type so that we can get the intersection.
...
It's clear the intersected genes explain more variance in PC1 compared with all 'UP' DEGs.
...
There's a wrapper function `filter_subset_sig()` which can directly do the whole Step 2 in one step. The result would be the same as running the above workflow step by step.

`filter_subset_sig()` is helpful when you have multiple datasets (more details in Section \@ref(multi-data)). It can output final signature after aggregating signature lists from all datasets.
...
Now the signature is specific to the target group, further refinement can be achieved by eliminating genes with strong signal in cancer cells or tissues, regarded as background expression.

mastR utilized a signal-to-noise ratio (SNR) approach to filter out genes with low SNR values which are considered to have low discriminative power between the group of interest and the background. This removes tumor purity as a factor in the identified signature markers.

Here the signal is the expression of signature in signal data and the noise is the expression of the signature in background data.

Two input datasets are needed:

- signal data (data for signature identification after process, `proc_data`) 
- background data (data of tissues or cancers of interest)
...
Customized data can be loaded as matrix, here use `ccle_crc_5` built in mastR as background data, use 'voomE' in `proc_data` as signal data.

`ccle_crc_5` is a DGEList object only contains 5 CRC cell line samples from [CCLE](https://depmap.org/portal/download/). More details for `help(ccle_crc_5)`.
...
The first step is to subset the samples of interest.
...
With the subset of matrices, low expression genes need to be removed first as SNR is computed using scaled expression across genes.

In this case, `sig_mat` from `proc_data` has been already filtered, only filter background data.

As `bg_mat` only has 5 samples, keep the genes with logTPM > 1 within more than 2 samples.

```{r bg data filtration}
## set cutoff for filtration
filter <- c(1, 2)
keep <- rowSums(bg_mat > filter[1], na.rm = TRUE) > filter[2]
bg_mat <- bg_mat[keep,]
```

## III) markers pool filtration

Now users can use `remove_bg_exp_mat()` to refine the markers pool.

Genes highly expressed in corresponding background data (in this case CRC adherent cell lines) will be removed. 

As markers and row names of `bg_mat` are gene SYMBOLs while `sig_mat` has ENSEMBL IDs as row names, mastR provides the gene ID conversion feature in most of the in-built functions by param `gene_id` to meet the mis-matched ID types of markers and input data.

`remove_bg_exp_mat()` allows to specify the gene ID types using parameter `gene_id`, the 1st element will be used as the ID type of signal data, and the 2nd one will be used for background data.
...
[CCLE](https://depmap.org/portal/download/) downloaded by `depmap::depmap_TPM()` is also available for the refinement.

As the whole CCLE is quite large, use `ccle_crc_5` to construct a small pseudo CCLE data with similar format.
...
Because `ccle` is a long data, subsetting needs to be done before converting data into matrix.
...
Then the same cutoff of filtration is used for `ccle`.
...
The refined markers pool is returned now. As we use `ccle_crc_5` to construct pseudo `ccle`, the result should be the same as above.
...
mastR provides a wrapper function `remove_bg_exp()` to do Step 3. I) II) III) all in one single function call.

CCLE long data is also accepted by setting param `bg_data = 'CCLE'` and pass CCLE data to param `ccle_tpm` and `ccle_meta`. Without the latter 2 parameters, users can use the whole CCLE from DepMap without loading it. More details in `help(remove_bg_exp)`.


## Combine with Signature

Then we combine the result with the signature `m_ct` we get from Step 2.

Use intersection to get the final signature, which is specific to the target group as well as less affected by the background expression (tissue purity).
...
Here, if we directly use `m_ct` instead of `Markers` as input `markers` for `remove_bg_exp_mat()` or `remove_bg_exp()`, then `m_ccl` is already the intersection result of original `m_ccl` and `m_ct`, so we can skip this.
...
- `sig_heatmap()` for signature heatmap across groups and its comparison with markers pool.  
    To validate if the signature shows clear and differential expression pattern between the target group and others, and if the pattern is clearer and cleaner after the refinement when compared with un-refined markers pool.
...
Obviously, the signature shows significantly higher score in NK cells.
...
Based on the scatter plot result, further filtration can be done to improve signature specificity by removing signature genes present in right bottom or left bottom region.
...
It's clear that the refined signature is enriched in all comparisons, and mastR successfully removed the long tail from the markers pool.
...
The advantage is that users can avoid over-normalizing or mis-correcting when carrying out data integration. And users can get more robust and conserved signature across datasets. It's better than "Remove Batch Effect" sometimes.

Of course, easy way such as "union" and "intersect" is also supported besides "RRA" (Robust Rank Aggregation), by setting `comb = union` or `comb = intersect`.
...
***Note:*** All signature visualization functions in mastR support multiple datasets. More details in `help` function.

Repeatedly use `im_data_6` as an example here.
...
After getting the signature, users can apply it in multiple ways. In this short demo, we will just show 3 simple applications of it, but not run here.

First simulate a scRNA data using `splatter` package. 
...
```{r get markers list, eval=FALSE}
markers_list <- lapply(rowData(data_sim)[,paste0("DEFacGroup", 1:4)],
                       \(x) rownames(data_sim[x > 1]))
```

Then use `pseudo_samples()` to get pseudo-bulk data and create a DGEList.

```{r pseudo bulking, eval=FALSE}
...
Use mastR wrapper function to get signature lists for each group. No markers pool of interest and no background here.
...
Show the overlapping of the signature lists generated by mastR and the markers lists from simulation.

```{r venn plot, eval=FALSE}
## venn plot
...
Use `sig_heatmap()` to compare the expression of signatures and markers. 

```{r app heatmap, eval=FALSE}
## heatmap
sig_heatmap(
  cpm(dge, log = TRUE),
...
Then randomly aggregate scRNA data to generate pseudo bulk data for scoring and deconvolution.
...
```{r add cellular prop, eval=FALSE}
...
Multiple scores are calculated by `singscore` package using signatures and markers. Use scatter plot to show the correlation between the score and the actual cellular proportion.

For signatures generated by mastR:

```{r score on signatures, eval=FALSE}
library(singscore)
...
Use `DWLS` package for cellular deconvolution.

Use signatures to subset scRNA reference data and build the signature profile on it. Cellular proportion can be obtained based on it.

```{r deconv, eval=FALSE}
sig_matrix <- DWLS::buildSignatureMatrixMAST(
  scdata = counts(data_sim)[unique(unlist(sig_ls)),],
  id = data_sim$Group,
  path = tempdir()
)
...
Use `singscore` as a marker-based annotation method, calculate the scores based on signatures and markers list, then assign each cell to the cell type with the highest score. Compare the result of signatures and markers list.

Using signatures:

```{r sig annotation, eval=FALSE}
library(singscore)
## normalization
...
```{r markers annotation, eval=FALSE}
