...
data("im_data_6", "NK_markers", "LM22", "LM7", "ccle_crc_5")
```
...
```{r load_collection, include=FALSE}
data("msigdb_subcollection")
```


```{r MSigDB genesets, fig.width=12, warning=FALSE}
MSig <- signature_from_collection(msigdb_collection, pattern = "NATURAL_KILLER_CELL_MEDIATED")
...
```{r integrate markers}
# How to create your own geneset
# setMethod("plot_genesets", "GeneSet", plot_genesets(...) code here)
# setMethod("plot_genesets", "GeneSetCollection", plot_genesets(x) code here)

nk_m <- NK_markers$HGNC_Symbol |> 
    GeneSet(geneIdType = SymbolIdentifier(), setName = "NK_markers")

Markers <- merge_genesets(
  NK_markers = nk_m,
  LM7 = LM[["LM7"]],
  LM22 = LM[["LM22"]],
  MSigDB = MSig,
...
If you need to use your own RNA-seq, you can format it as ...
...
# original -> process_data -> procdata, 
# (original, procdata) -> plot_diagnostics, 
# procdata -> select_features(design?, summary=T/F) -> 2 Genesets & summary
```

## Step before / After processing data
We need to do PCA to ...

```{r PCA for im_data_6, fig.width=10, fig.height=7}
## PCA on raw counts data with standard edgeR normalization
pca_matrix_plot(im_data_6, group_by = "celltype:ch1", counts = TRUE)


# original -> process_data -> procdata

## PCA on limma::voomed data after standard edgeR normalization
pca_matrix_plot(proc_data, group_by = "celltype.ch1", counts = FALSE)
```
...
# We can also ensure that markers can separate our target_group
...
# No need to depend on org.Hs.eg.db

## convert ensembl IDs into symbols to match markers pool
deg_up <- mapIds(org.Hs.eg.db::org.Hs.eg.db, DEG_6$UP, "SYMBOL", "ENSEMBL")
...
Or you can do the whole thing in one step.
```{r markers for cell type, eval=FALSE}
...
The example uses `CCLE_tpm` from [DepMap CCLE](https://depmap.org/portal/download/), containing RSEM quantified TPM data of 1392 cancer cell lines (until 2022.04). For fast test, here we only selected 5 CRC cell line samples as a small test dataset `ccle_crc_5`.
...
* `ID` param is the group vector of input samples, or the name of group column.
...
```{r markers for cancer cell line, eval=FALSE}
...
# m_ccl <- filter_non_tissue(type = "colorectal",
#                            markers = geneIds(Markers))
