...
Then the same filtration cutoff of `bg_mat` as above is used for `ccle`.
...
Wrapper function `remove_bg_exp()` can complete Step 3. I) II) III) all in one single function.
...
Obviously, the signature shows significantly higher score level in NK cells.
...
If the signature is highly specific to NK cells, most of the signature genes should appear in left-top region (scaled expression > 1 in NK, < 1 in others). The gene present in right-top region should represent the co-expression in both groups.
...
Further optimization can be done by removing signature genes present in the right bottom or left bottom region based on the scatter plot.
...
The refined signature is enriched in all comparisons and mastR successfully removed the long tail from the markers pool.
...
The standard workflow of mastR usage is complete.

But the above is just a show case of single dataset, mastR can also be applied to multiple datasets.

After the Step 2c (Section \@ref(intersect-pool)), for:

1) single dataset  
    Return the final result as the signature.
...
Instead of integrating datasets into one larger dataset and fit it into one linear model, mastR refines signature in each dataset respectively and the aggregation method can be defined using param `comb` (e.g. `comb = "RRA"`).

The advantage of this is to avoid over-normalizing or mis-correcting when carrying out data integration. More robust and conserved signature across datasets can be obtained. It's better than "Remove Batch Effect" sometimes.

Simple combination methods like "union" and "intersect" are also supported (e.g. `comb = union`).

It's better to use "union" when only a few genes are refined for each dataset, while "RRA" is better for robust gene selection from large DEG lists, and "intersect" is proper for highly overlapping gene lists.

***Note:***

All signature visualization functions in mastR support multiple datasets. More details in `help()`.

To demonstrate the usage on multiple datasets, repeatedly use `im_data_6` as an example.

```{r refine multiple datasets, eval=FALSE}
...
Although mastR is designed for bulk RNA-seq data, to make use of abundant scRNA resource, mastR provides `pseudo_samples()` function to help convert scRNA-seq data into pseudo-bulk data, which can then be used in mastR workflow.

`pseudo_samples()` can aggregate cells into pseudo-bulk samples according to the given factor(s) using param `by`. Then use `filter_subset_sig()` to get the signature from scRNA-seq data.
...
Users can apply the signature in multiple ways. In this short demo, we will just show 3 simple applications of it, but not run here.
...
Use mastR wrapper function `filter_subset_sig()` to get the signature for each group. No markers pool of interest and no background here.
...
Show the overlapping of the signature lists generated by mastR and the markers lists from the simulation.
...
Use signatures to subset scRNA reference data and build its signature profile. Cellular proportion can be estimated based on it.
...
Use `singscore` as a marker-based annotation method. Calculate the scores based on the signatures and markers lists, then assign each cell to the cell type with the highest score. Compare the result based on signatures and markers lists.
